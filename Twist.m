...
y = inputdlg(prompt, dlg_title, 1, defaultans); % Input of perturbation
...
% Difference between final and initial x- and y- coordinates
z = zeros(e,2);
for i=1:e
...
while max(abs(z))>0.0001
    for i=1:e
        ...
        delta_y = 0;
        for j=1:n_nodes
            delta_x = delta_x + neighbour(i,j)*(Nodes_f(j,1) - Nodes_f(i,1));
            delta_y = delta_y + neighbour(i,j)*(Nodes_f(j,2) - Nodes_f(i,2));
        end
        z(i,1) = delta_x;
        z(i,2) = delta_y;
    end
    Nodes_f(1:e,1:2) = Nodes_f(1:e,1:2) + alpha*z;
    t = t+1;
end
...
clc
%%
Comp = [Nodes_i(1:e,1:2), Nodes_f(1:e,1:2)]
t
